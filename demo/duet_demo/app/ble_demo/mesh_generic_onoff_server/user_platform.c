...
//pwm config
duet_pwm_dev_t g_sonata_pwm = {0};

void app_hal_init(void)
{
      g_sonata_pwm.port = PWM_OUTPUT_CH1;
 //   duet_pwm_pinmux_init(&g_sonata_pwm);
}

void toggle_led(void)
{
    if(0 == g_sonata_pwm.config.duty_cycle)
    {
        g_sonata_pwm.port = PWM_OUTPUT_CH1;
        g_sonata_pwm.config.duty_cycle = 1;
        g_sonata_pwm.config.freq = 1000;
        duet_pwm_init(&g_sonata_pwm);
        duet_pwm_start(&g_sonata_pwm);
    }
    else
    {
        g_sonata_pwm.port = PWM_OUTPUT_CH1;
        g_sonata_pwm.config.duty_cycle = 0;
        g_sonata_pwm.config.freq = 1000;
        duet_pwm_init(&g_sonata_pwm);
        duet_pwm_start(&g_sonata_pwm);
    }
