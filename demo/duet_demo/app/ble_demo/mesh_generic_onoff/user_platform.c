...
#if PLF_UART1
    duet_uart_set_callback(UART1_INDEX, at_handle_uartirq);
...
CRITICAL_FUNC_SEG void assert_err(int cond)
{
...
* @return void
****************************************************************************************/
CRITICAL_FUNC_SEG void assert_param(int param0, int param1)
{
    // print assert message
...
* @return void
****************************************************************************************/
CRITICAL_FUNC_SEG void assert_warn(int param0, int param1)
{
    // print assert message
...
* @return void
****************************************************************************************/
CRITICAL_FUNC_SEG void dump_data(uint8_t* data, uint16_t length)
{
...
//pwm config
duet_pwm_dev_t g_duet_pwm = {0};

static duet_gpio_dev_t mesh_button;
static uint32_t mesh_button_value = 0;
static duet_timer_dev_t antishake_timer;

void onoff_msg_send(void *arg);
void antishake_timer_cb(void *arg);
extern void app_mesh_send_msg(void *arg);


void app_hal_init(void)
{
    printf("app_hal_init\r\n");
    duet_pinmux_config(PAD10,PF_PWM0);

    mesh_button.port = GPIO11_INDEX;
    mesh_button.config = DUET_INPUT_PULL_UP;
    mesh_button.priv = NULL;
    duet_gpio_init(&mesh_button);
    duet_gpio_enable_irq(&mesh_button, DUET_IRQ_TRIGGER_FALLING_EDGE, onoff_msg_send, NULL);

    antishake_timer.config.arg=NULL;
    antishake_timer.config.cb=antishake_timer_cb;
    antishake_timer.config.period=10000;
    antishake_timer.config.reload_mode=TIMER_RELOAD_AUTO;
    antishake_timer.port=DUET_TIMER1_INDEX;
    antishake_timer.priv=NULL;
}
void onoff_msg_send(void *arg)
{
    duet_timer_init(&antishake_timer);
    duet_timer_start(&antishake_timer);
}

void antishake_timer_cb(void *arg)
{

    duet_gpio_input_get(&mesh_button,&mesh_button_value);
    if(mesh_button_value == 0)
    {
        app_mesh_send_msg(NULL);
        duet_timer_stop(&antishake_timer);
    }
}
...
{
    if(0 == g_duet_pwm.config.duty_cycle)
    {
        g_duet_pwm.port = PWM_OUTPUT_CH1;
        g_duet_pwm.config.duty_cycle = 1;
        g_duet_pwm.config.freq = 1000;
        duet_pwm_init(&g_duet_pwm);
        duet_pwm_start(&g_duet_pwm);
    }
    else
    {
        g_duet_pwm.port = PWM_OUTPUT_CH7;
        g_duet_pwm.config.duty_cycle = 0;
        g_duet_pwm.config.freq = 1000;
        duet_pwm_init(&g_duet_pwm);
        duet_pwm_start(&g_duet_pwm);
    }
