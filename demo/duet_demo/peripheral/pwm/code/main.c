...
duet_pwm_dev_t duet_pwm = {0};
void duet_pwm_test(void)
{
    duet_pwm.port = PWM_OUTPUT_CH0;
    duet_pwm.config.freq = 1000;
    duet_pwm.config.duty_cycle = 0.5;
    duet_pwm_init(&duet_pwm);
    duet_pwm_start(&duet_pwm);
...
{
    static uint8_t times = 0,change = 0;
    ...
    if(change == 0)
    {
        if(times%2)
        {
            duet_pwm.config.freq = 10000;
            duet_pwm.config.duty_cycle = 0.75;
            duet_pwm_para_chg(&duet_pwm,duet_pwm.config);
        }
        else
        {
            duet_pwm.config.freq = 1000;
            duet_pwm.config.duty_cycle = 0.5;
            duet_pwm_stop(&duet_pwm);
            duet_pwm.port += 1;
            if(duet_pwm.port > 7)
                duet_pwm.port = 0;
            duet_pwm_init(&duet_pwm);
    ...
    else if(change == 1)
    {
        ...
        {
            duet_pwm.config.freq = 1000;
            duet_pwm.config.duty_cycle = 0;
            duet_pwm_para_chg(&duet_pwm,duet_pwm.config);
        }
        else
        {
            duet_pwm.config.freq = 1000;
            duet_pwm.config.duty_cycle = 1;
            duet_pwm_stop(&duet_pwm);
            duet_pwm.port += 1;
            if(duet_pwm.port > 7)
                duet_pwm.port = 0;
...
void duet_timer_test(void)
{
    ...
    duet_timer.config.period = 2000000;
