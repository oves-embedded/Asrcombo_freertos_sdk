...
int8_t duet_uart_set_rx_fifo_threshold(UART_TypeDef * UARTx, uint8_t uart_fifo_level)
{
    /* params assertion */

    /* set level*/
    UARTx->IFLS &= ~(0x7 << 3);
    UARTx->IFLS |= (uart_fifo_level << 3);

    return 0;
}

int8_t duet_uart_set_tx_fifo_threshold(UART_TypeDef * UARTx, uint8_t uart_fifo_level)
{
    /* params assertion */

    /* set level*/
    UARTx->IFLS &= ~(0x7);
    UARTx->IFLS |= (uart_fifo_level);
...
int32_t duet_uart_set_baudrate(duet_uart_dev_t* uart)
{
    UART_TypeDef* UARTx = NULL;
    if(DUET_UART0_INDEX == uart->port)
    {
...
int32_t duet_uart_finalize(duet_uart_dev_t *uart)
{
    ...
    {
        tmp_value = REG_RD(PERI_CLK_DIS_REG1) & (~(UART1_BUS_CLK_EN|UART1_PERI_CLK_EN));
        REG_WR(PERI_CLK_DIS_REG1, (tmp_value | (UART1_BUS_CLK_EN|UART1_PERI_CLK_EN)));
    }
    else
    {
        tmp_value = REG_RD(PERI_CLK_DIS_REG1) & (~(UART2_BUS_CLK_EN|UART2_PERI_CLK_EN));
        REG_WR(PERI_CLK_DIS_REG1, (tmp_value | (UART2_BUS_CLK_EN|UART2_PERI_CLK_EN)));
    }
