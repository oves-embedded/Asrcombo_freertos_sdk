...
void duet_pwm_cfg(duet_pwm_dev_t *pwm)
{
    ...
    switch(pwm->port)
    {
        case PWM_OUTPUT_CH0:
            ...
            tmp_value |= (PWM_CLOCK / pwm->config.freq - 1);
            PWM->PWM01LOAD = tmp_value;

            tmp_value = PWM->PWM0CMP & (~(0x0000FFFF));
            tmp_value |= (uint16_t)(PWM_CLOCK / pwm->config.freq * (1-pwm->config.duty_cycle));
        ...
        case PWM_OUTPUT_CH1:
            ...
            tmp_value |= (uint16_t)(PWM_CLOCK / pwm->config.freq * (1-pwm->config.duty_cycle)) << 16;
            PWM->PWM0CMP = tmp_value;
            PWM->PWM01DB = 0;
            break;
        case PWM_OUTPUT_CH2:
            ...
            tmp_value |= ((uint16_t)(PWM_CLOCK / pwm->config.freq - 1) << 16);
            PWM->PWM01LOAD = tmp_value;
