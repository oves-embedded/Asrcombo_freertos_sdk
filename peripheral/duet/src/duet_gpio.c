                            ...
                            duet_gpio_irq_handler_t handler, void *arg)
{
    ...
    if(gpio->port < DUET_GPIO_NUM_PER_GROUP)
    {
        switch(trigger)
        {
            ...
            case DUET_IRQ_TRIGGER_FALLING_EDGE:
                ...
                break;
            case DUET_IRQ_TRIGGER_HIGH_LEVEL:
                GPIO_GROUP0->INTTYPECLR = (1 << gpio->port); //edge or level trig
                GPIO_GROUP0->INTPOLSET = (1 << gpio->port); //trig polarity
                break;
            case DUET_IRQ_TRIGGER_LOW_LEVEL:
    ...
    tmp_value = REG_RD(DUTE_IRQ_EN_REG) & (~GPIO_IRQ_BIT);
    REG_WR(DUTE_IRQ_EN_REG, (tmp_value | (GPIO_IRQ_BIT)));
