...
#if ( ( portSTACK_GROWTH > 0 ) || ( configRECORD_STACK_HIGH_ADDRESS == 1 ) )
        pxTaskStatus->pxStackEnd = pxTCB->pxEndOfStack;
#endif
        pxTaskStatus->pxStackTop = (StackType_t *)pxTCB->pxTopOfStack;
...
#define BLE_ROM_CODE_START  0x00003000
#define BLE_ROM_CODE_END    0x0000f5d8

uint32_t OsIsExeAddr(uint32_t addr)
{
    if ((addr >= (uint32_t)&_flash_text_start) && (addr <= (uint32_t)&_flash_text_end)) {
        return TRUE;
    } else if ((addr >= (uint32_t)&_itcm_start) && (addr <= (uint32_t)&_itcm_end)) {
       return TRUE;
...
extern char _estack[];
uint32_t OsGetMainStackHighWaterMark()
{
    return prvTaskCheckFreeStackSpace( ( uint8_t * )_stack_base);
}
        ...
        if( pxTaskStatusArray != NULL )
        {
            ...
            for( x = 0; x < uxArraySize; x++ )
            {
                ...
                sprintf( pcWriteBuffer, "\t%c\t%u\t%u\t%u\t%u\r\n",
                        ...
                        ( unsigned int ) (((( unsigned int )(pxTaskStatusArray[ x ].pxStackEnd + 1) + portBYTE_ALIGNMENT) & ( ~( ( portPOINTER_SIZE_TYPE ) portBYTE_ALIGNMENT_MASK ) )) - ( unsigned int )pxTaskStatusArray[ x ].pxStackBase),
            ...
            /* main stack watermark&info */
            pcWriteBuffer = prvWriteNameToBuffer( pcWriteBuffer, "main_stack" );
            sprintf(pcWriteBuffer, "\t%c\t%u\t%u\t%u\t%u\r\n",
                    lega_rtos_is_in_interrupt_context() ? 'H':'T', /* H: handler mode T: thread mode */
                    (unsigned int)configMAX_PRIORITIES,
                    (unsigned int)_Min_Stack_Size,
                    (unsigned int)OsGetMainStackHighWaterMark() * sizeof(StackType_t),
                    (unsigned int)0
                    );
            pcWriteBuffer += strlen( pcWriteBuffer );
#if 0   //unused
            ...
            for (x = 0; x < uxArraySize; x++) {
                ...
                sprintf(pcWriteBuffer, "Task %s:\n", pxTaskStatusArray[ x ].pcTaskName);
                pcWriteBuffer += strlen( pcWriteBuffer );
                for(uint32_t stk = (uint32_t)pxTaskStatusArray[ x ].pxStackTop, index = 0; stk <= (uint32_t)pxTaskStatusArray[ x ].pxStackEnd && index < 500; stk += 4)
                {
                    if(OsIsExeAddr(*(uint32_t*)stk))
                    {
                        sprintf(pcWriteBuffer, "[%08x]:0x%x\n", (unsigned int)stk, (unsigned int)*(uint32_t*)stk);
                        pcWriteBuffer += strlen( pcWriteBuffer );
                    }
                    index++;
                }
            ...
            {
                sprintf(pcWriteBuffer, "TASK main_stack:\n");
                pcWriteBuffer += strlen( pcWriteBuffer );
                for(uint32_t stk = __get_MSP(), index = 0; stk <= (uint32_t)&_estack && index < 500; stk += 4)
                {
                    if(OsIsExeAddr(*(uint32_t*)stk))
                    {
                        sprintf(pcWriteBuffer, "[%08x]:0x%x\n", (unsigned int)stk, (unsigned int)*(uint32_t*)stk);
            ...
            sprintf(pcWriteBuffer, "---------------------- Task Stack End ----------------------\n");
            pcWriteBuffer += strlen( pcWriteBuffer );
#endif
