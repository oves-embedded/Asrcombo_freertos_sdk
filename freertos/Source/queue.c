...
BaseType_t xQueueWaitingTaskisEmpty( QueueHandle_t xQueue )
{
    Queue_t * const pxQueue = ( Queue_t * ) xQueue;

    return listLIST_IS_EMPTY( &( pxQueue->xTasksWaitingToReceive ) );
}
...
Queue_t * const pxQueue = ( Queue_t * ) xQueue;
    ...
    uxSavedInterruptStatus = portSET_INTERRUPT_MASK_FROM_ISR();
    {
        ...
        {
            printf("uxMessagesWaiting=%ul, uxLength= %ul",(unsigned int)pxQueue->uxMessagesWaiting, (unsigned int)pxQueue->uxLength);
...
char *curTaskName;
    ...
    if(pxQueue == 0)
    {
        curTaskName = pcTaskGetTaskName(xTaskGetCurrentTaskHandle());
        printf("curtask %s\n",curTaskName);
    }
    configASSERT( pxQueue );
