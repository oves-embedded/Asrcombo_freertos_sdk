...
{
#ifdef __FREERTOS_PAL__
    int32_t in_domain = 2, in_type = 0, in_protocol = 0;
    int ret = 0;
    ...
    ret = socket(in_domain, in_type, in_protocol);

#ifdef JOYLINK_SOCKET_DEBUG_INFO
    jl_platform_printf("\r\njl_platform_socket->in_domain: %d in_type: %d in_protocol: %d\r\n", in_domain,in_type,in_protocol);
    jl_platform_printf("njl_platform_socket->ret :%d\n", ret);
#endif
...
int32_t jl_platform_fcntl(int32_t s, JL_FCNTL_CMD_T cmd)
{
    ...
    if (cmd == JL_FCNTL_CMD_SETFL_O_NONBLOCK) {
        if ((flags = fcntl(s, F_GETFL, 0)) == -1) {//add thrid para to "0"
            ret = -1;
        } else {
...
int32_t jl_platform_bind(int32_t s, const jl_sockaddr *my_addr, uint32_t addrlen)
{
    ...
    int32_t ret=bind(s, &addr, addrlen);

#ifdef JOYLINK_SOCKET_DEBUG_INFO
    jl_platform_printf("jl_platform_bind->ret: %d\n", ret);
#endif
    return ret;
#else
    return -1;
...
int32_t jl_platform_setsockopt(int32_t socket, JL_SOCK_OPT_LEVEL_T level, JL_SOCK_OPT_NAME_T option_name, const void *option_value, int32_t option_len)
{
    ...
    if (option_name == JL_SOCK_OPT_NAME_SO_SNDTIMEO) {
        in_option_name = SO_SNDTIMEO;
    }

    int32_t ret = setsockopt(socket, in_level, in_option_name, option_value, option_len);

#ifdef JOYLINK_SOCKET_DEBUG_INFO
    jl_platform_printf("jl_platform_setsockopt->ret: %d\n", ret);
#endif
