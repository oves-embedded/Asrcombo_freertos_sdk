...
/******************************************************************************
 DEFINE CONSTANTS
 ******************************************************************************/
#define  F_JLPINFO_NAME  "f_jlpinfo_name"
...
#else
user_dev_status_t user_dev =   //参考example
{
    .Power = 1,
    .Mode = 0,
    .State = 0,
 ...
 * Private Data
****************************************************************************/
static char jlp_mac_addr[12] = {0}, jlp_read_mac_flag = 0;
static char s_feedid[18]={0};
static uint8_t s_return_st_app_flag = 1;
 ...
 * @returns:
 */
void joylink_dev_reset_return_st_flag(void)
{
  s_return_st_app_flag = 1;
}
    ...
    */
    ...
    if((s_return_st_app_flag)&&(st == 2))
    {
      ...
      BLE SDK 正常流程是依次这四个状态更新APP的UI,其中：
        ...
        */
      s_return_st_app_flag = 0;
      //3. E_JL_NET_CONF_ST_WIFI_CONNECT_START.
      jl_send_net_config_state(E_JL_NET_CONF_ST_IOT_CONNECT_SUCCEED, (uint8_t*)s_feedid, sizeof(s_feedid));
      //4. E_JL_NET_CONF_ST_WIFI_CONNECT_SUCCEED
...
#ifdef _SAVE_FILE_
    //Sample code for saving JLP info in flash.
    lega_flash_kv_set(F_JLPINFO_NAME, (void *)jlp, sizeof(JLPInfo_t), 1);//写KV值数组进flash
#endif
    ret = E_RET_OK;

#ifdef JOYLINK_EXTERN_DEBUG_INFO
    jl_platform_printf("joylink_dev_set_attr_jlp:\n");
    jl_platform_printf("jlp->feedid: %s\n", jlp->feedid);
    jl_platform_printf("jlp->accesskey: %s\n", jlp->accesskey);
    jl_platform_printf("jlp->localkey: %s\n", jlp->localkey);
    jl_platform_printf("jlp->joylink_server: %s\n", jlp->joylink_server);
    jl_platform_printf("jlp->server_port: %d\n", jlp->server_port);

    jl_platform_printf("jlp->gURLStr: %s\n", jlp->gURLStr);
    jl_platform_printf("jlp->gTokenStr: %s\n", jlp->gTokenStr);

    jl_platform_printf("jlp->mac: %s\n", jlp->mac);
    jl_platform_printf("jlp->prikey: %s\n", jlp->prikey);
    jl_platform_printf("jlp->is_actived: %d\n", jlp->is_actived);
    jl_platform_printf("jlp->model_code_flag: %d\n", jlp->model_code_flag);
    jl_platform_printf("jlp->version: %d\n", jlp->version);
    jl_platform_printf("jlp->devtype: %d\n", jlp->devtype);
    jl_platform_printf("jlp->lancon: %d\n", jlp->lancon);
    jl_platform_printf("jlp->cmd_tran_type: %d\n", jlp->cmd_tran_type);
#endif
    jl_platform_memcpy(s_feedid, jlp->feedid, sizeof(s_feedid));//暂时backup feedid到s_feedid
...
joylink_dev_get_idt(jl2_d_idt_t *pidt)
{
    ...
    */
    ...
    // jl_platform_strcpy(pidt->rand, user_idt.rand); //这个不用修改。
...
joylink_dev_get_user_mac(char *out)
{
    ...
    */
#ifdef JOYLINK_EXTERN_DEBUG_INFO
    jl_platform_printf("joylink_dev_get_user_mac->jlp_read_mac_flag: %d\n", jlp_read_mac_flag);
#endif

    if (jlp_read_mac_flag == 0)
    {
        uint8_t jlp_mac_addr_temp[6];

        lega_wlan_get_mac_address(jlp_mac_addr_temp);

#ifdef JOYLINK_EXTERN_DEBUG_INFO
        jl_platform_printf("joylink_dev_get_user_mac->mac: ");
        for (uint8_t i = 0; i < 6; i++)
        {
            jl_platform_printf(" %02x", jlp_mac_addr_temp[i]);
        }
        jl_platform_printf("\r\n");
#endif
        for(uint8_t i=0; i<6; i++)
        {
            jl_platform_sprintf(jlp_mac_addr+i*2,"%02X", jlp_mac_addr_temp[i]);
        }

#ifdef JOYLINK_EXTERN_DEBUG_INFO
        jl_platform_printf("joylink_dev_get_user_mac->jlp_mac_addr: %s\n", jlp_mac_addr);
#endif
        jl_platform_strcpy(out, jlp_mac_addr);
...
joylink_dev_get_private_key(char *out)
{
    ...
    */
    ...
    jl_platform_strcpy(out, F_PRIVATE_KEY);
    return E_RET_OK;
#else
    return E_RET_ERROR;
#endif
...
joylink_dev_get_jlp_info(JLPInfo_t *jlp)
{
    ...
    */
    ...
    int32_t fjlp_size = sizeof(JLPInfo_t);
    lega_flash_kv_get(F_JLPINFO_NAME, &fjlp, &fjlp_size);//读取KV值数组
...
#ifdef JOYLINK_EXTERN_DEBUG_INFO
    jl_platform_printf("joylink_dev_get_jlp_info:\n");
    jl_platform_printf("jlp->feedid: %s\n", jlp->feedid);
    jl_platform_printf("jlp->accesskey: %s\n", jlp->accesskey);
    jl_platform_printf("jlp->localkey: %s\n", jlp->localkey);
    jl_platform_printf("jlp->joylink_server: %s\n", jlp->joylink_server);
...
{
    return joylink_dev_http_post(host, query, revbuf, buflen);
}
