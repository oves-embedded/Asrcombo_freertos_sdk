...
/**
 * @defgroup SONATA_PRF_HOGPD_API PRF_HOGPD_API
 * @{
 */
...
#define SONATA_HOGPD_NB_REPORT_INST_MAX            (11)
...
{
    /// Request from peer device to Read or update a report value
    uint16_t (*hogpd_report_request)(uint8_t conidx, uint8_t operation, uint16_t handle, uint8_t report_type, uint8_t hid_idx, uint16_t length, uint8_t idx, uint8_t *value);
    /// Inform Device APP that Protocol Mode Characteristic Value has been written on Device
    uint16_t (*hogpd_protocol_mode_request)(uint8_t conidx, uint8_t hid_idx, uint8_t operation, uint8_t proto_mode);
    /// Inform APP that HID Control Point Characteristic Value has been written on Device
    uint16_t (*hogpd_control_point_write)(uint8_t conidx, uint8_t hid_idx,uint8_t new_ctnl_pt);
 ...
 * @param
 * @return  connection state @see  hid_connection_state
 */
int sonata_get_connection_state();

/*!
 * @brief sonata_set_connection_state
 * @param state set connection state @see  hid_connection_state
 */
 ...
 * @param report  buffer of report data
 * @return SONATA_GAP_ERR_INVALID_PARAM , API_SUCCESS
 */
 ...
 * @param conidx Connection Index
 * @param hid_idx  index of hid, for single HID, set it to 0
 * @param status  Status of the request  GAP_ERR_NO_ERROR or ATT_ERR_APP_ERROR
 * @param proto_mode new protocol mode characteristic value , 0 for BOOT mode 1 for REPORT mode
 * @return API_SUCCESS
 */
 ...
 * @param conidx Connection Index
 * @param hid_idx index of hid, for single HID, set it to 0
 * @param report_idx report index in HID report map , 0 for boot reports and report map
 * @param report_type report type  @see sonata_hogpd_report_type
 * @param report_map_len report map length
 * @param report_map report map
 * @return  API_SUCCESS
 */
int sonata_prf_hogpd_send_report_map_confirm(uint8_t conidx, uint8_t hid_idx, uint8_t report_idx, uint8_t report_type, uint16_t report_map_len, const uint8_t *report_map);

/*!
 * @brief sonata_prf_hogpd_add  add hogpd profile
 * @param sec_lvl  Security Leve  @see gapm_profile_task_add_cmd
 * @param start_hdl Service start handle  @see gapm_profile_task_add_cmd
 * @param db_cfg   Parameters saved of  hogpd profile
 */
void sonata_prf_hogpd_add(uint8_t sec_lvl, uint16_t start_hdl, sonata_hogpd_db_cfg_t db_cfg);

/*!
 * @brief sonata_prf_hogpd_init init hogpd profile
 * @param
 */
void sonata_prf_hogpd_init(void);

/*!
 * @brief sonata_prf_hogpd_enable enable hogpd profile
 * @param conidx   Connection Index
 */
 ...
 * @param report_idx report index of mouse in HID report map
 * @param report   report data of mouse
...
/*!
 * @brief sonata_prf_hogp_send_report_update_request_for_keyboard  report keyboard data
 * @param report_idx    index of keyboard report data in HID report map
 * @param ps_keyboard_report_t    buffer for   report data of keyboard
 * @param report_len              length of   report data of keyboard
 * @return
 */
void sonata_prf_hogp_send_report_update_request_for_keyboard(uint8_t report_idx, uint8_t *ps_keyboard_report_t, uint16_t report_len);


/*!
 * @brief sonata_prf_hogpd_register_callback  callbacks of the request and indicate from host hid
 * @param cb callbacks for hogpd value
 * @return API_SUCCESS
 */
 ...
 * @param hid_idx index of hid, for single HID, set it to 0
 * @param report_type report type  @see sonata_hogpd_report_type
 * @param idx report index in HID report map
 * @return
 */
void sonata_prf_hogpd_boot_mouse_input_report(uint8_t conidx, uint8_t hid_idx, uint8_t report_type, uint8_t idx);

/*!
 * @brief sonata_prf_hogpd_report send report to hid host
 * @param conidx Connection Index
 * @param hid_idx index of hid, for single HID, set it to 0
 * @param report_idx report index in HID report map
 * @param report_type report type  @see sonata_hogpd_report_type
 * @param length length of  report data
 * @param value   buffer of report data
 * @param send_cfm  true or false
 * @return
 */
void sonata_prf_hogpd_report(uint8_t conidx, uint8_t report_type, uint8_t hid_idx, uint8_t report_idx, uint16_t length, uint8_t *value, bool send_cfm);

/*!
 * @brief sonata_prf_hogpd_proto_mode_req  modify hogp_boot_prot_mode
 * @param conidx Connection Index
 * @param operation type of operation requested by peer device @see sonata_hogpd_op  SONATA_HOGPD_OP_PROT_UPDATE will be correct
 * @param proto_mode  new protocol mode characteristic value , 0 for BOOT mode 1 for REPORT mode
 * @return
 */
...
/*!
 * @brief sonata_prf_hogpd_updata_conn_params update connection  params
 * @param intv_min    Connection interval minimum
 * @param intv_max    Connection interval maximum
 * @param latency     Latency
 * @param time_out   Supervision timeout
 */
...
/*!
 * @brief sonata_prf_hogpd_reset  erase fLash hogpd profile saved
 */
void sonata_prf_hogpd_reset();

/*!
* @brief sonata_hogpd_send_error_code
* @param conidx Connection Index
* @param operation type of operation requested by peer device @see hogpd_op
* @param hid_idx index of hid, for single HID, set it to 0
* @param report_type report type  @see sonata_hogpd_report_type
* @param report_idx report index in HID report map
...
/*!
 * @brief send report confirm message
 * @param conidx        connection id
 * @param operation     see sonata_hogpd_op
 * @param hid_idx       index of hid, for single HID, set it to 0
 * @param report_type   see sonata_hogpd_report_type
 * @param report_len    report length
 * @param report_idx    index of the report in HID report map
 * @param report_value  report data
 */
void sonata_hogpd_send_report_confirm(uint8_t conidx, uint8_t operation, uint8_t hid_idx, uint8_t report_type, uint16_t report_len, uint8_t report_idx, const uint8_t *report_value);

/** @}*/

///@cond
#endif // defined(STACK_BLE_HOST_PRESENT) || defined(BLE_HOST_PRESENT)
///@endcond
