...
struct sonata_gap_dev_name
{
    ...
    uint16_t length;
    /// name value
    uint8_t value[__SONATA_ARRAY_EMPTY];
};
...
/*@TRACE*/
typedef struct sonata_gap_bdaddr
{
...
/*@TRACE*/
typedef struct sonata_gap_per_adv_bdaddr
{
    /// BD Address of device
    uint8_t addr[SONATA_GAP_BD_ADDR_LEN];
    /// Address type of the device 0=public/1=private random
    ...
    /// Advertising SID
    uint8_t adv_sid;
} sonata_gap_per_adv_bdaddr_t;

/// Resolving list device information
/*@TRACE*/
typedef struct sonata_gap_ral_dev_info
{
    /// Device identity
