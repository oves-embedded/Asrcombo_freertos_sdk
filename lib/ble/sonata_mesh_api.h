...
typedef enum
{
    /// Mesh store data except sequence number
    MESH_STORE_ALL_EXCP_SEQ,
    /// Mesh store iv and sequence
    MESH_STORE_IV_SEQ,
    /// Mesh store pub information
    MESH_STORE_PUB,
    /// Mesh store sub information
    MESH_STORE_SUB,
    /// Mesh store data all
    MESH_STORE_DATA_ALL,
}mesh_store_sector_t;
...
typedef enum
{
  ...
  /// mesh msg replay number
  MESH_MSG_NUMBER_REPLAY,
  /// mesh model replay number
  MESH_NUMBER_REPLAY,
  /// mesh scan interval
  MESH_SCAN_INTERVAL,
  /// mesh non addr type
  MESH_NON_ADV_ADDR_TYPE,
  /// mesh lpn poll attempt number
  MESH_LPN_POLL_ATTEMPT_NUM,
  /// mesh con adv interval
  MESH_CON_ADV_INTERVAL,
  /// mesh scan addr type
  MESH_SCAN_ADDR_TYPE,
  /// mesh buffer number default :32
  /// it is best not to exceed 48
  MESH_BUFFER_NUM,
...
typedef enum
{
 ...
 /// Indicate Attation Time
  ...
  /// Indicate  replay list is full
  MESH_REPLAY_LIST_FULL_IND,
}mesh_core_evt_ind_t;
...
typedef enum
{
 ...
 /// App Input Auth Data OOB Info
  MESH_AUTH_DATA_SET,
  /// Setting Set PROV data
  MESH_SET_LOCAL_PROV_DATA,
 ...
 MESH_SET_LOCAL_FRIEND_STATE,
     /// Update Local Netkey
 MESH_UPDATE_LOCAL_NETKEY,
     /// Update Local Appkey
 MESH_UPDATE_LOCAL_APPKEY,
       /// Setting Local iv index and seq no
 MESH_SET_LOCAL_SEQ,
     /// Unbind AppKey
 MESH_SET_LOCAL_MODEL_APP_UNBIND,
...
typedef union
{
    ...
    ///mesh relay state
    mesh_relay_param_t relay_state;
    ///mesh default ttl
    ...
    ///mesh scan param
    mesh_scan_param_t scan_param;
    ///mesh con adv interval
    uint16_t con_adv_interval;
    ///mesh number of addresses that can be stored for Message Replay Protection
    uint16_t nb_addr_msg_replay;
    ///mesh model repaly number
    uint8_t replay_number;
    ///mesh non adv address type
    uint8_t non_adv_addr_type;
    ///mesh scan address type
    uint8_t scan_addr_type;
    ///mesh lpn poll attempt number
    uint8_t lpn_poll_attempt_num;
    ///mesh buffer number
    uint8_t mesh_buffer_num;
...
typedef struct
{
    ...
    ///The sequence number of this message.
     uint32_t seq_number;
    /// The IV index of this message.
    uint32_t iv_index;
     /// ttl of the message (Required for a response)
    uint8_t ttl;
    /// Message length
    uint16_t msg_len;
    /// Message content
...
typedef struct
{
    ...
    uint16_t msg_len;
    /// Message content
    uint8_t  msg[__SONATA_ARRAY_EMPTY];
} mesh_model_msg_param_t;

/// Vendor Model request to publish a new message
typedef struct mesh_vendor_model_msg_param
{
    uint8_t element;
    int8_t   mic_64;
    /// TTL
    ...
    /// Message content
    uint8_t  msg[__SONATA_ARRAY_EMPTY];
} mesh_vendor_model_msg_param_t;

/// Mesh Model Message Sent Callback
typedef  void (* model_vendor_msg_sent_cb)(mesh_vendor_model_msg_param_t * p_param,STATUS status) ;
...
typedef struct
{
    ...
    uint16_t length;
    /// Value
    uint8_t val[__SONATA_ARRAY_EMPTY];
...
*/
typedef struct mesh_set_local_prov_data
{
...
///Mesh  Set Local Iv Index And Seq NO
typedef struct mesh_set_local_iv
{
    uint32_t iv;
    uint8_t state;
}mesh_set_local_iv_t;

///Mesh  Set Local  Seq NO
typedef struct mesh_set_local_seq
{
    uint32_t seq;
}mesh_set_local_seq_t;

///Mesh  Set Local  Seq NO
typedef struct mesh_set_local_addr
{
    uint16_t addr;
}mesh_set_local_addr_t;
...
typedef union
{
    ...
    ///structure for subs ind
    mesh_sub_addr_ind_t subs_ind;
    ///structure for pub ind
    mesh_publish_addr_ind_t pub_ind;
...
typedef union
{
    ...
    mesh_set_local_prov_data_t           local_prov_data_set;
    ///provisioner appkey struct
    ...
    ///Mesh  Set Local iv
    mesh_set_local_iv_t    local_iv;
    ///Mesh  Set Local seq
    mesh_set_local_seq_t    local_seq;
    ///Mesh  Start lpn
    ...
    ///Mesh  prim addr
    mesh_set_local_addr_t local_addr;
...
///Mesh Start Complete fuc
typedef  void (* mesh_stop_complete)(void) ;
...
/**
 ***********************************************************
 * @brief Func  stop mesh protocol
 * @param[in]  cb    stopped callback @see mesh_stop_complete
 ************************************************************
*/
void sonata_mesh_stop(mesh_stop_complete cb);
...
/**
 ***********************************************************
 * @brief Func  get fisrt vendor model id in the element
 * @retval uint32_t model id
 ************************************************************
*/
uint32_t sonata_mesh_get_vendor_model_id(uint8_t element);

/**
 ***********************************************************
 * @brief Func  get model local handle
 * @param[in]  element     element pos
 * @param[in]  modelid     model id
 * @return uint16_t
 * @retval model handle    (0xFFFF means not found)
 ************************************************************
*/
uint16_t  sonata_mesh_get_model_handle(uint8_t element,  uint32_t modelid);

/**
 ***********************************************************
 * @brief Func  model handle to model info
 * @param[in]  model_handle   model local handle
 * @param[out]  element  element index
 ...
 ************************************************************
*/
STATUS  sonata_mesh_get_element_modelid_by_handle(uint16_t model_handle, uint8_t *element,  uint32_t *modelid);

/**
 ***********************************************************
 * @brief call this func when mesh ota start
 * @return void
 ************************************************************
*/
void sonata_mesh_notify_ota_start(void);

/**
 ***********************************************************
 * @brief  call this func when mesh ota stop
 * @return void
 ************************************************************
*/
void sonata_mesh_notify_ota_stop(void);
...
/**
 ***********************************************************
 * @brief Func  prov adv onoff
 * @param[in] ctl 1:open  0:close
 ************************************************************
*/
void sonata_mesh_prov_broadcast(uint8_t ctl);

/**
 ***********************************************************
 * @brief  mesh get default ttl
 * @return ttl value
 ************************************************************
*/
uint8_t sonata_mesh_get_default_ttl(void);

/**
 ***********************************************************
 * @brief  mesh get number of elements
 * @return element number
 ************************************************************
*/
uint8_t sonata_mesh_get_nb_elements(void);
/**
 ***********************************************************
 * @brief  mesh get IV Index
 * @return IV
 ************************************************************
*/
uint32_t sonata_mesh_get_iv(void);
/**
 ***********************************************************
 * @brief  mesh get sequence number
 * @return sequence number
 ************************************************************
*/
uint32_t sonata_mesh_get_seq(void);
/**
 ***********************************************************
 * @brief  clear mesh nvds data by sector
 * @return status
 ************************************************************
*/
STATUS sonata_mesh_clean_sector_data(void (*clean_cb)(void),uint8_t sector);

/**
 ****************************************************************************************
 * @brief Clear Replay Manager list all
 *
 ****************************************************************************************
 */
void sonata_mesh_clear_replay_list(void);
/**
 ****************************************************************************************
 * @brief Clear Replay Manager list info by address
 *
 * @param[in] src_addr      Source address
 ****************************************************************************************
 */
void sonata_mesh_clear_replay_list_by_addr(uint16_t src_addr);
/**
 ****************************************************************************************
 * @brief Unbind model all appkey
 *
 * @param[in] element_index      Element index
 * @param[in] model_id           model id
 ****************************************************************************************
 */
void sonata_mesh_model_unbind_all(uint16_t element_index, uint32_t model_id);
